...
void CPrivateSendClient::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Smartiecoin related functionality
...
{
    if(fLiteMode) return; // disable all Smartiecoin specific functionality
    ...
    RenameThread("smartiecoin-ps-client");
